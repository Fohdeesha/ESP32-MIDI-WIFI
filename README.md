...
```
USB MIDI device
      â”‚  USB OTG host mode (ESP-IDF usb_host stack)
      â–¼
USB MIDI class driver â”€â”€â–º MIDI event queue (FreeRTOS)
                                â”‚
                                â–¼
...
- The S3's USB OTG peripheral (GPIO19/20) is used for host mode, so serial
  console and flashing go through the board's UART port â€” use the connector
...
- **SSID:** `ESP32-MIDI-Setup` Â· **password:** `midimidi`
...
- [x] 0.1.0 â€” project skeleton: builds, boots, prints version
- [x] 0.2.0 â€” WiFi station connect + mDNS advertisement + WS2812 status LED
- [x] 0.3.0 â€” RTP-MIDI session established, test notes sent
- [x] 0.4.0 â€” web config UI + OTA firmware updates + setup-AP fallback
- [ ] 0.5.0 â€” USB host: enumerate USB MIDI device, parse MIDI-over-USB packets
- [ ] 0.6.0 â€” full bridge: USB MIDI in â†’ RTP-MIDI out
- [ ] 0.7.0 â€” reverse direction (RTP-MIDI in â†’ USB MIDI out)
- [ ] 0.8.0 â€” session initiator mode (invite a configured peer by IP:port)
